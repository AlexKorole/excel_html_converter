...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _handle_create(self):
        ...
        if "multipart/form-data" not in content_type or not boundary_match:
            self._send_json({"error": "Некорректный запрос"}, status=400)
            return
        ...
        if not name_field or not file_field or not file_field.get("filename"):
            self._send_json({"error": "Укажите имя отчёта и выберите xlsx-файл"}, status=400)
            return

        name = name_field["value"].decode("utf-8", errors="replace").strip()
        if not name:
            self._send_json({"error": "Имя отчёта не может быть пустым"}, status=400)
            return

        include_connected = "include_connected" in fields

        def _optional_int_field(field_name, label):
            ...
            try:
                ...
                return None, f"{label} должен быть числом"
            if value < 1:
                return None, f"{label} должен быть больше 0"
            return value, None

        pivot_limit, pivot_error = _optional_int_field("pivot_limit", "лимит строк для сводной")
        if pivot_error:
            self._send_json({"error": pivot_error}, status=400)
            return

        table_limit, table_error = _optional_int_field("table_limit", "лимит строк для таблицы")
        if table_error:
        ...
        def _watch(proc, rid):
            ...
            if m and m.get("status") == "processing":
                m["status"] = "error"
                m["error"] = (
                    f"Процесс сборки завершился неожиданно (код {proc.exitcode}), "
                    "вероятная причина — нехватка памяти на большом файле. "
                    "Попробуйте задать лимит строк."
                )
                write_meta(rid, m)
